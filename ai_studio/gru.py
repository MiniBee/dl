    ...
    with open(mig_file) as f1, open(inf_file) as f2, open(density_file) as f3:
        ...
        for i, line in enumerate(f2):
            if i == 0:
                continue
            line_list = line.split(',')
            line_list = [float(x.strip()) / 10 for x in line_list[1:]]
            y.append(line_list)
...
if __name__ == '__main__':
    ...
    m1 = Model(N_HIS, 64, 392, 'mse', batch_size=32, epochs=100, opt=adam)
    m1.build_model()
    ...
    m2 = Model(N_HIS, 64, 392, loss=rmsle, batch_size=32, epochs=400, opt=adam)
