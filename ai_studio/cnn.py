...
def data_process1(mig_file, inf_file, density_file):
    ...
    x1 = np.concatenate([x, y, d], axis=2)
    for i, a in enumerate(x):
        if i >= N_HIS:
            m_x.append(x1[i - N_HIS: i])
            m_y.append(x[i])
            i_y.append(y[i])
            d_y.append(d[i])

    return np.array(m_x), np.array(m_y), np.array(i_y), np.array(d_y), np.array(x1), np.array(y), np.array(d)
...
class Model():
    ...
    def build_model(self):
        ...
        outputs = tf.keras.layers.Conv2D(filters=5, kernel_size=(2, 1), padding='same', activation='relu')(inputs)
        outputs = tf.keras.layers.Conv2D(filters=1, kernel_size=1, activation='relu')(outputs)
        outputs = tf.keras.layers.Lambda(self.re_)(outputs)
        outputs = tf.keras.layers.GRU(self.hidden_num, dropout=0.1, recurrent_dropout=0.1)(outputs)
...
if __name__ == '__main__':
    mx, my, iy, dy, x, y, d = data_process1(file_path + '/region_migration.csv', file_path + '/infection.csv', file_path + '/density.csv')
    
    # mx_train, mx_test, my_train, my_test = train_test_split(mx, my, test_size=0.3)
    # adam = tf.keras.optimizers.Adam(0.001)
    # m1 = Model(N_HIS, 32, 392, 'mse', batch_size=32, epochs=400, opt=adam)
    # m1.build_model()
    ...
    m2 = Model2(N_HIS, 64, 392, loss=rmsle, batch_size=32, epochs=800, opt=adam)
    ...
    for i, a in enumerate(mx):
        print('-' * 50)
        print(m2.model.predict(a[np.newaxis, :, :])[0][1])
        print(iy[i][1])
    
    # mx_train, mx_test, dy_train, dy_test = train_test_split(mx, dy, test_size=0.3)
    # adam = tf.keras.optimizers.Adam(0.001)
    # m3 = Model(N_HIS, 32, 392, 'mse', batch_size=32, epochs=500, opt=adam)
    # m3.build_model()
