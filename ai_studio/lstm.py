...
if __name__ == '__main__':
    mx, my, iy, x = data_process1(file_path + '/region_migration.csv', file_path + '/infection.csv')
    ...
    with open(file_path + '/test.csv', 'w') as f:
        for i in range(len(x) - N_HIS):
            f.write(','.join([str(x) for x in m1.predict(x[i: i + N_HIS][np.newaxis, :, :])[0]]) + '\n')
    ...
    # m2 = Model(N_HIS, 32, 392, loss=rmsle, batch_size=32, epochs=400, opt=adam)
