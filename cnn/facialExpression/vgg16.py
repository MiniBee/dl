...
class Model():
    ...
    def build_model(self):
        ...
        with self.graph.as_default():
            # 人脸数据
            self.inputs = tf.placeholder(dtype=tf.float32, shape=[None, 48, 48, 1], name="inputs") 
            # 表情序列，用0-6数字表示
            # self.target = tf.placeholder(tf.int32, 
            #                              [self.batch_size],
            #                              name="target") 
            self.target_onehot = tf.placeholder(dtype=tf.float32, shape=[None, 7], name="target_onehot")
